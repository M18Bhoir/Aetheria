...
import { HiOutlinePlusCircle, HiOutlinePencil, HiOutlineTrash } from "react-icons/hi"; // Icons

const placeholderImage = (text = 'No Image') => `https://placehold.co/150x100/EEE/31343C?text=${encodeURIComponent(text)}`;
...
function MyListings() {
    ...
    const fetchMyItems = async () => {
        ...
        setError('');
        try {
            const res = await api.get('/api/marketplace/my');
            setItems(res.data || []);
        } catch (err) {
    ...
    // TODO: Function to handle changing status (Available/Sold/Reserved)
    const handleStatusChange = async (itemId, newStatus) => {
         console.log(`Attempting to change status of ${itemId} to ${newStatus}`);
         // Implement API call to PUT /api/marketplace/:id with { status: newStatus }
         try {
            const res = await api.put(`/api/marketplace/${itemId}`, { status: newStatus });
    ...
    return (
        <div className="p-4 md:p-6 dark:text-gray-100">
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-2xl md:text-3xl font-bold text-gray-900 dark:text-white">My Marketplace Listings</h1>
                 <button
            ...
            {loading && <p className="text-center text-gray-500 dark:text-gray-400">Loading your listings...</p>}
            ...
            {!loading && !error && items.length === 0 && (
             ...
             {!loading && !error && items.length > 0 && (
                <div className="space-y-4">
                    {items.map((item) => (
                        <div key={item._id} className="p-4 border border-gray-200 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800 shadow-sm flex flex-col md:flex-row md:items-center gap-4">
                           {/* Image */}
                           <img
                            ...
                            <div className="flex-grow">
                                <Link to={`/dashboard/marketplace/${item._id}`} className="text-lg font-semibold text-blue-600 dark:text-blue-400 hover:underline">{item.title}</Link>
                                <p className="text-sm text-gray-500 dark:text-gray-400">{item.category} - {item.condition}</p>
                                <p className="text-md font-bold text-green-600 dark:text-green-400 mt-1">₹{item.price.toLocaleString('en-IN')}</p>
                            ...
                            {/* Actions */}
                             <div className="flex-shrink-0 flex flex-col items-start md:items-end space-y-2">
                                ...
                                <div className="flex items-center space-x-2">
                                     <label htmlFor={`status-${item._id}`} className="text-xs font-medium text-gray-500 dark:text-gray-400">Status:</label>
                                     <select
                                         id={`status-${item._id}`}
                                         value={item.status}
                                ...
                                <div className="flex space-x-2">
                                    <button
                                        ...
                                        className="p-1 text-yellow-500 hover:text-yellow-700 disabled:opacity-50" title="Edit Listing (Coming Soon)">
                                        <HiOutlinePencil className="h-5 w-5"/>
                                    </button>
                                     <button
                                        onClick={() => handleDelete(item._id)}
                                        className="p-1 text-red-500 hover:text-red-700" title="Delete Listing">
